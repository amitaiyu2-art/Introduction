...
while True:
        ...
        if not conversation.history:
            ...
            for idx, (text, polarity, sentiment_type) in enumerate (conversation_history, start=1):
                if sentiment_type == "Positive":
                    ...
                    color = Fore.YELLOW
                    emoji = ":|"
                print(f"{idx}. {color}{emoji} {text}" f"Polarity: {polartiy:.2f}, {sentiment_type}{Style.RESET_ALL}")
        continue
    ...
    if polarity > 0.25:
        ...
        sentiment_type = "Neautral"
        color = Fore.YELLOW
        emoji = ":|"

    conversation_history.append((user_input), polarity, sentiment_type)

    print(f"{color}{emoji}{sentiment_type} sentiment detected! " f"Polarity: {polarity:2.f}")
